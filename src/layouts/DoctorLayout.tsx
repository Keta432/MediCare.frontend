...
const DoctorLayout: React.FC<DoctorLayoutProps> = ({ children }) => {
  ...
  const menuItems = [
    ...
    { icon: FaClipboardList, label: 'Appointments', path: '/doctor/appointments' },
    { icon: FaClipboardList, label: 'Follow-ups', path: '/doctor/followups' },
    { icon: FaUserInjured, label: 'Patients', path: '/doctor/patients' },
    { icon: FaChartBar, label: 'Reports', path: '/doctor/reports' },
    { icon: FaChartPie, label: 'Analytics', path: '/doctor/analytics' },
    { icon: FaComments, label: 'Messages', path: '/doctor/messages' },
    { icon: FaUserMd, label: 'Profile', path: '/doctor/profile' },
  ];
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="flex items-center space-x-4">
              ...
              <div className={cn(
                ...
                <h2 className="text-lg font-semibold text-gray-800 truncate">Dr. {user?.name}</h2>
                <p className="text-sm text-gray-500">{user?.specialization || 'Doctor'}</p>
              </div>
            </div>
          </div>
